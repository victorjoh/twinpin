...
import Graphics.Rasterific.Transformations (rotateCenter)
import SDL hiding
  ( lerp,
    (^*),
    (^+^),
...
createPlayer pos direction color joystickId =
  Player
...
fillShape color shape =
  withTexture (uniformTexture color) $
    fill $
      map
        ( either
            (CubicBezierPrim . scaleAndOffset playerRadius)
...
boostTimeToAimShadowLength maxShadowWidth boostTime =
  maxShadowWidth
    * if boostTime > boostRechargeTime
      then
        fromIntegral (fullBoostCycle - boostTime)
...
aimShadowShape len
  ...
  | len > 0 && len < 1 / 3 =
    let t = len * 3
     ...
     in [ Left cutTopLeftCurve,
          Left cutBottomLeftCurve,
          Right $
            Line
              (_cBezierX3 cutBottomLeftCurve)
  ...
  | len > lineLength + 1 / 3 && len < 4 / 3 =
    let t = (len - lineLength - 1 / 3) / (1 - lineLength)
     ...
     in [ Left topLeftCurve,
          ...
          Right $
            Line
              (_cBezierX3 cutBottomRightCurve)
...
createAngle x y oldAngle
  | isCloseToDefault x && isCloseToDefault y = oldAngle
  | otherwise = atan2 y x
  where
    isCloseToDefault direction = abs direction < minAxisPosition

createVelocity :: Direction1D -> Direction1D -> Velocity2D
createVelocity x y
  | isCloseToDefault x && isCloseToDefault y = V2 0 0
  | otherwise = V2 x y
  where
...
updatePlayer events dt obstacles player =
  let Player shape movement gun vitality playerId = player
   ...
   in Player newCircle newMovement newGun vitality newPlayerId
...
updateMovement dt buttonEvents axisEvents aim movement =
  let Aim2D _ _ aimAngle = aim
      ...
      newVelocity
        | newBoostTime > boostTime =
          if newDirection /= V2 0 0
      ...
      velocities =
        if wasBoosting && willNotBeBoosting
          then
...
updateJoystick events joystickId =
  if hasJoystickRemovedEvent events joystickId
    then Nothing
    else Just joystickId

hasJoystickRemovedEvent :: [Event] -> JoystickID -> Bool
hasJoystickRemovedEvent events joystickId =
  any
...
fireBullet events bulletId player =
  let Player (Circle position _) _ gun _ (PlayerId _ joystickId) = player
      ...
      (newReloadTime, maybeBullet) =
        if reloadTime == 0 && newState == Firing
          then
            ( minFiringInterval,
...
eventToGunState (Left (AxisEvent axisId amountPressed)) =
  if axisId == rightTriggerButtonId
    then
      if amountPressed > triggerMinFireValue
        then Just Firing
        else Just Idle
    else Nothing
