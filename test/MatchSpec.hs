...
spec = do
  ...
  describe "drawMatch" $ do
    it "converts from match to something that can be drawn by SDL" $
      let player = createPlayer (V2 48 350) 0 Red Nothing
          ...
          match =
            Match
              (Movables 1 [bullet] [intersectedPlayer])
              (Obstacles (createBounds 800 600) [pillar])
       in -- just test the order, the tests for the individual draw
          -- functions test that the position is right
          map fst (drawMatch match)
            `shouldBe` map
              fst
              ( [drawBullet bullet]
                  ++ drawPlayer player
  ...
  describe "updateMatch" $ do
    ...
    it "can create a bullet" $
      let position = V2 48 350
       ...
       in getBullets new `shouldBe` [createBullet position direction 8]
    it "cannot create 2 bullets from the same player in rapid succession" $
      let player =
       ...
       in length (getFirstIntersecting new) `shouldBe` 1
    it "can move a bullet" $
      let bullet = createBullet (V2 400 300) (pi / 2) 0
          old =
            Match
              (Movables 1 [bullet] [])
              (Obstacles (createBounds 800 600) [])
       ...
       in map getBulletPosition (getBullets new)
    ...
    it "changes color of bullets that hit players" $
      let bullet = createBullet (V2 100 300) 0 0
          ...
          old =
            Match
              (Movables 1 [bullet] [player])
              (Obstacles (createBounds 800 600) [])
          new = updateMatch [] (getBulletMovementTime 100) old
       in map getBulletState (getBullets new) `shouldBe` [HasHitPlayer]
      ...
      let bullet = createBullet (V2 100 300) 0 0
          ...
          old =
            Match
              ...
              (Obstacles (createBounds 800 600) [])
          new = updateMatch [] (getBulletMovementTime 100) old
       in getHealth (getFirstPlayer new)
    ...
    it
      ...
      $ let bullet = createBullet (V2 400 300) 0 7
            sourcePlayer =
              IntersectedPlayer [7] $
      ...
      let -- the bullet will not hit
          ...
          old =
            Match
              ...
              (Obstacles (createBounds 800 600) [])
          new = updateMatch [] (getBulletMovementTime 100) old
       in map getBulletState (getBullets new)
      ...
      let -- the bullet will not hit
          ...
          old =
            Match
              ...
              (Obstacles (createBounds 800 600) [])
          new = updateMatch [] (getBulletMovementTime 100) old
       in getHealth (getFirstPlayer new) `shouldBe` playerMaxHealth
    it
    ...
    it
      ...
      $ let bullet = createBullet (V2 100 300) 0 0
            ...
            new =
              updateMatch [] 1 $
                updateMatch [] (getBulletMovementTime 100) old
         in map getHealth (getPlayers new)
              `shouldBe` replicate 2 (playerMaxHealth - bulletDamage)
    it "can collide two players" $
      let player1 = createPlayer (V2 100 300) 0 Red (Just 0)
  ...
  describe "assignJoysticksToMatch" $ do
    let updateWith oldJoysticks addedJoysticks =
          let players =
                map
