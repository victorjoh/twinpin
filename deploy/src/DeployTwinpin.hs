...
addEntry modifiedTime srcPath entryPath (LinuxDistribution entries) = do
  ...
  where
    setModifiedTime t e = e {Tar.entryTime = t}
...
addSdlLibrary modifiedTime (LinuxDistribution entries) =
  addEntry
    modifiedTime
    linuxReadmeSrc
    linuxReadmeTarget
...
addSdlLibrary modifiedTime (WindowsDistribution entries) = do
  ...
  extraLibFiles <-
    fmap concat $
      mapM (traverseToSnd' listDirectory . strip) $
        splitOn
          ","
          libDirs
  sdl2Src <-
    liftMaybe
      ("Failed to find SDL2.dll, looked in: " ++ show libDirs)
      (return . uncurry (</>))
